...
import { GetSeaporteAssuranceList, UpdateSeaporteAssurance } from "src/redux/actions/SeaporteActions";
...
export const SeaporteQualityAssurance = (props) => {

  const SeaporteAssuranceData = useSelector(
    (state) => state.seaporte?.seaporte_assurance_list?.success,
  );

  const updateData=useSelector((state)=>state.seaporte.seaporte_assurance_update);
  ...
  useEffect(() => {
    if (SeaporteAssuranceData && SeaporteAssuranceData.length > 0) {
      setState(SeaporteAssuranceData[0]);
    }
  }, [SeaporteAssuranceData]);
  ...
  useEffect(() => {
    dispatch(GetSeaporteAssuranceList());
  }, [dispatch,updateData]);
  ...
  const SubmitData = async () => {
    ...
    formData.append("id", SeaporteAssuranceData[0]?.id);
    formData.append("image", state.image);
    formData.append("head_line", state.head_line);
    formData.append("description", state.description);
    console.log(state, "stateeeeeeeeeeeeee");
    dispatch(UpdateSeaporteAssurance(formData));
  };
  console.log(SeaporteAssuranceData, "SeaporteAssuranceData");
