...
export default function LatestNews(props) {
    ...
    const UpdatePopUpSubmitData = async (e) => {
        ...
        setupdateModelopen(false)
        await axios
            .post(`${baseUrlPostGres()}/api/letest-news-update`, formData, {
                headers: apiHeader()
            })
            .then(response => {
    ...
    const listCalling = () => {
        axios
            .get(`${baseUrlPostGres()}/api/list_news`, {
                headers: apiHeader()
            })
            .then(response => {
              if(response.data.message==="Unauthorized please login again!"){
                ...
                <Box sx={{ flexGrow: 1 }}>
                    <Grid container spacing={2}>
                        <Grid xs={12} style={{ display: "flex", justifyContent: "space-between" }}>
                            ...
                            />
                            <Button variant="contained" style={{ margin: "50px" }} alignRight="true" onClick={handleOpen}>Add News</Button>
                        </Grid>
                    </Grid>
