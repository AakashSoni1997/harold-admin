...
export default function Notice() {
    ...
    useEffect(() => {
        axios
            .get(`${baseUrlPostGres()}/api/career-notice-list`, {
                headers: apiHeader()
            })
            .then(response => {
    ...
    const handleChange = (e) => {
        ...
        if (name === "content_description" || name === "descrption" || name == "codes") {
            state[name] = value
    ...
    const SubmitData = async () => {
        await axios
            .put(`${baseUrlPostGres()}/api/career-notice-update`, JSON.stringify(
                {
                    "id": state.id,
                    "descrption": state.descrption,
                    "tittle": state.tittle
                }
            ), {
                headers: apiHeader()
        ...
        <Box sx={{ width: '100%', typography: 'body1' }}>
            ...
            {state &&
                <div className="App">
                    ...
                    <CKEditor
                        initData={state.descrption}
                        onInstanceReady={() => {
                            console.log("fjksdhfsdfsfs")
                        }}
                        name="descrption"
