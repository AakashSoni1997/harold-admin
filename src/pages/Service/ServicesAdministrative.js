...
export const ServicesAdministrative = (props) => {
  ...
  const Slidercapabilities = useSelector(
    (state) => state.services?.services_administrative_list?.success,
  );
  const updateData = useSelector(
    (state) => state.services?.services_administrative_update,
  );
  ...
  useEffect(() => {
    dispatch(GetServicesAdministrativeList());
  }, [dispatch, updateData]);
  ...
  const UpdatePopUpSubmitData = async (e) => {
    ...
    setupdateModelopen(false);
    dispatch(UpdateServicesAdministrativeList(formData));
  };
