...
import {
  GetServicesCapabilitiesList,
  UpdateServicesCapabilitiesList,
...
export default function ServicesCapabilities(props) {
  ...
  const Slidercapabilities = useSelector(
    (state) => state.services?.services_capabilities_list?.success,
  );
  const updateData = useSelector(
    (state) => state.services?.services_capabilities_update,
  );
  ...
  useEffect(() => {
    dispatch(GetServicesCapabilitiesList());
  }, [dispatch, updateData]);
  ...
  const UpdatePopUpSubmitData = async (e) => {
    ...
    setupdateModelopen(false);
    dispatch(UpdateServicesCapabilitiesList(formData));
  };
